...
def compute_psfs(
    ...
    for point in iterable_coords:
        W = compute_pupil_phase(
            ...
            X=-Fx * scale_factor,
            Y=-Fy * scale_factor,
            u=(point[0]) / (samples / 2),  # was point[0]
            v=-(point[1]) / (samples / 2),  # was
        )  # point[0]/(sys_params['samples']/2)
        ...
        # curr_psf = torch.tensor(
        #     shift(
        #         curr_psf.cpu(),
        #         (-(point[1].cpu()), (point[0].cpu())),
        #         mode="constant",
        #         cval=0,
        #     )
        # ).to(device)
        # curr_psf[curr_psf < 0] = 0
...
def generate_psf_dataset(path, sys_params, coeffs, type="grid"):
    ...
    [Fx, Fy] = torch.tensor(
        np.meshgrid(fx, fx)
    )  # torch.meshgrid ordered ij not xy so we use numpy instead
    ...
    if type == "grid":
        ...
        for row, v in tqdm(zip(range(samples), v_norm)):
            for col, u in zip(range(samples), u_norm):
                ...
                H[circle < 1e-12] = 0
                curr_psf = torch.fft.ifft(
                    torch.fft.ifft(H, dim=0), dim=1
                )  # torch.fft.ifftn(H,) WTF IS THIS BUG POWERS OF 2 ARE BROKEN?!!!
                # curr_psf = torch.fft.ifftn(H)
                curr_psf = torch.roll(
                    curr_psf,
                    shifts=(curr_psf.shape[0] // 2, curr_psf.shape[1] // 2),
                    dims=(0, 1),
                )
                curr_psf = torch.square(torch.abs(curr_psf))
                curr_psf = fun.pad(
                ...
                curr_psf = torch.roll(
                    curr_psf,
                    (-int(row - samples / 2), int(col - samples / 2)),
                    dims=(0, 1),
                )
                curr_psf = curr_psf[
        ...
        for index, (u, v) in enumerate(zip(u_norm, v_norm)):
            ...
            curr_psf = torch.roll(
                coherent_psf,
                shifts=(-coherent_psf.shape[0] // 2, -coherent_psf.shape[1] // 2),
                dims=(0, 1),
            )
            curr_psf = torch.square(torch.abs(curr_psf))
            curr_psf = fun.pad(
            ...
            curr_psf = sp.ndimage.shift(
                curr_psf, (-index / 2, index / 2), prefilter=False
            )
            curr_psf = curr_psf[
                int(samples / 2) : -int(samples / 2),
                int(samples / 2) : -int(samples / 2),
...
def main():
    ...
    compute_psfs(
        coeffs, desired_list=[(0, 0), (0.5, 0.5), (1, 1)], sys_params=sys_params
    )
